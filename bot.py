...
@app.route("/")
def index():
    return "✅ Debate Bot is running and healthy!"
        ...
        if data["remaining"] > 0:
            ...
            if 0 < data["remaining"] <= 30 and data["remaining"] % 10 == 0 and data["remaining"] != last_alert:
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"━━━━━━━━━━━━━━━━\n⏳ انتبه! {color} {data['current_speaker']} تبقى {format_time(data['remaining'])} على انتهاء المداخلة!\n━━━━━━━━━━━━━━━━"
                )
        else:
            if not data.get("extra_mode", False):
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"━━━━━━━━━━━━━━━━\n🚨 انتهى وقت {color} {data['current_speaker']}!\n⏱️ بدأ حساب الوقت الزائد...\n━━━━━━━━━━━━━━━━"
                )
            await asyncio.sleep(1)
            data["extra_time"] += 1
            color = "🔴"
            if data["extra_time"] <= 30:
                if data["extra_time"] % 10 == 0 or data["extra_time"] == 1:
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=f"━━━━━━━━━━━━━━━━\n⌛ الوقت الزائد للمتحدث الحالي {color} {data['current_speaker']}: +{format_time(data['extra_time'])}\n━━━━━━━━━━━━━━━━"
                    )
            else:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"━━━━━━━━━━━━━━━━\n⏱️ توقف وقت {color} {data['current_speaker']}!\n🚨 يجب تبديل المحاور...\n━━━━━━━━━━━━━━━━"
                )
                data["running"] = False
                break
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data["step"] == "running":
        ...
        if text_conv in ["تبديل", "تنازل"]:
            ...
            if next_speaker not in data["summary"]:
                data["summary"][next_speaker] = {"used": 0, "over": 0, "turns": 0}
            ...
            if text_conv == "تنازل":
                await context.bot.send_message(chat_id=chat_id,
                    text=f"━━━━━━━━━━━━━━━━\n🚨 تنازل {color_prev} {prev_speaker} عن المداخلة!\n🔁 الدور ينتقل الآن إلى: {color_next} {next_speaker}\n━━━━━━━━━━━━━━━━")
            else:
                await context.bot.send_message(chat_id=chat_id,
                    text=f"━━━━━━━━━━━━━━━━\n🔁 تم التبديل إلى: {color_next} {next_speaker}\n"
        ...
        if text_conv == "اعادة":
            ...
            data["extra_time"] = 0
            data["extra_mode"] = False
            await update.message.reply_text(f"♻️ تم إعادة الجولة الحالية للمتحدث {data['current_speaker']} إلى {data['duration']//60}د")
            return
...
if __name__ == "__main__":
    print("🚀 Debate Bot running on Flask")
